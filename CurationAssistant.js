...
function buildPopulationsText(populations) {
  return populations
    .map(
      (pop) =>
        `${GNOMAD_POPULATIONS[pop.code] || pop.code} ${pop.ac}\t${pop.an}\t${pop.hom}\t${
          pop.afRaw || toPercent(pop.ac, pop.an)
        }`
    )
    .join("\n");
}
...
function buildVariantIntroLine(data, variantType) {
  ...
  if (variantType === "frameshift") {
    ...
    const fromAa = parsed?.fromFull || "amino acid";
    return `This heterozygous ${gene} ${data.cdna} variant is predicted to cause a frameshift and leads to a premature termination codon ${
      data.downstreamAaCount || "YYY"
    } amino acids downstream ${fromAa}. This alteration is predicted to lead to a truncated or absent protein, and truncating variants in ${gene} are known to be pathogenic.`;
  }
  ...
  if (variantType === "intronic") {
    return `This heterozygous ${gene} ${data.cdna} variant occurs in the ${getIntronicSpliceRegion(data.cdna)} splice region of intron ${
      data.intronNumber || "X"
    } of the ${gene} gene.`;
  }
  ...
  if (variantType === "inframe_deletion") {
    return `The ${data.cdna} (${data.protein}) in-frame deletion variant${rs} involves residues in a tract of ${
      data.inframeResidueCount || "XXX"
    } residues ${data.inframeKnownFunction || "without known function"} [PM4/BP3].`;
  }

  if (variantType === "inframe_duplication") {
    return `The ${data.cdna} (${data.protein}) in-frame duplication variant${rs} involves residues in a tract of ${
      data.inframeResidueCount || "XXX"
    } residues ${data.inframeKnownFunction || "without known function"} [PM4/BP3].`;
  }

  if (variantType === "inframe_insertion") {
    return `The ${data.cdna} (${data.protein}) in-frame insertion variant${rs} involves residues in a tract of ${
      data.inframeResidueCount || "XXX"
    } residues ${data.inframeKnownFunction || "without known function"} [PM4/BP3].`;
  }

  if (variantType === "inframe_indel") {
    return `The ${data.cdna} (${data.protein}) in-frame deletion/duplication/insertion variant${rs} involves residues in a tract of ${
      data.inframeResidueCount || "XXX"
    } residues ${data.inframeKnownFunction || "without known function"} [PM4/BP3].`;
  }
...
function buildSpliceAiLine(data) {
  ...
  return `Computational splice prediction tool Splice AI (https://spliceailookup.broadinstitute.org/) suggests that this variant may impact splicing, although this prediction has not been confirmed by experimental studies. Alamut tools have identified a potential splice alteration near ${
    data.cdna || "this position"
  }.`;
}
...
function buildClinVarLine(data) {
  ...
  if (parts.length === 0) {
    return `This variation has been documented in the ClinVar database${
      data.variationId ? ` (Variation ID: ${data.variationId})` : ""
    }.`;
  }

  return `This variation has been documented in the ClinVar database with records suggesting classification as ${parts.join(" or ")}${
    data.variationId ? ` (Variation ID: ${data.variationId})` : ""
  }.`;
}
